...
class Script
{
    ...
    public static function GenerateList($db, $name){
        ...
        foreach ($users as $r){
            if($r['username'] == $name){
                $list_name = '<h5 class="mb-1" style="color: #d9534f;"><strong>' . $r['username'] . '</strong></h5>';
            ...
            print('<a href="#" id="item-' . $r['username'] .'" class="list-group-item list-group-item-action flex-column align-items-start">
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            <button id="btn-' . $r['username'] .'" class="btn btn-lg my-2 my-sm-0 mr-sm-2 '. $btn_class .'" type="submit" style="height: 100%; width: 100%;" onclick="ModifyUser(\'' . $r['username'] . '\', \'' . $r['email'] . '\', \'div-modify-user\')">Modify</button>
                        </div>
                    </div>
                </a>');
        }
